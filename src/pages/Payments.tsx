...
type SortField = 'payment_number' | 'order_number' | 'customer' | 'payment_type' | 'amount_usd' | 'currency' | 'exchange_rate_used' | 'payment_date' | 'created_by';
...
export function Payments() {
  ...
  const [currentRate, setCurrentRate] = useState({ rate_cash: 1330, rate_installment: 1580 });
  ...
  const fetchPayments = useCallback(async () => {
    ...
    if (sortField === 'order_number') {
      result.sort((a, b) => {
        const aV = String((a.order as Record<string, unknown>)?.order_number || '');
        const bV = String((b.order as Record<string, unknown>)?.order_number || '');
        return sortDir === 'asc' ? aV.localeCompare(bV) : bV.localeCompare(aV);
      });
    } else if (sortField === 'customer') {
      result.sort((a, b) => {
        const key = language === 'ku' ? 'full_name_ku' : 'full_name_en';
        const aV = String(((a.order as Record<string, unknown>)?.customer as Record<string, string>)?.[key] || '');
        const bV = String(((b.order as Record<string, unknown>)?.customer as Record<string, string>)?.[key] || '');
        return sortDir === 'asc' ? aV.localeCompare(bV) : bV.localeCompare(aV);
      });
    } else if (sortField === 'created_by') {
      result.sort((a, b) => {
        const aV = String((a.created_by_profile as Record<string, string>)?.full_name_en || '');
        const bV = String((b.created_by_profile as Record<string, string>)?.full_name_en || '');
        return sortDir === 'asc' ? aV.localeCompare(bV) : bV.localeCompare(aV);
      });
    }
  ...
  useEffect(() => { setPage(1); }, [search, filterType, filterDate, sortField, sortDir]);
  ...
  const getExchangeRate = () => formData.payment_type === 'installment' ? currentRate.rate_installment : currentRate.rate_cash;
  ...
  const handleAddPayment = async () => {
    ...
    if (formData.payment_type === 'installment' && payData?.id) {
      ...
      if (pendingEntries && pendingEntries.length > 0) {
        ...
        const links: { payment_id: string; installment_entry_id: string; allocated_amount_usd: number }[] = [];
        let firstEntryId: string | null = null;

        for (const entry of pendingEntries) {
          if (budget <= 0.001) break;
          const due = entry.amount_usd - (entry.paid_amount_usd || 0);
          if (due <= 0) continue;

          if (!firstEntryId) firstEntryId = entry.id;

          if (budget >= due) {
            const allocated = Math.round(due * 100) / 100;
            budget = Math.round((budget - due) * 1000) / 1000;
            await supabase.from('installment_entries').update({
              paid_amount_usd: entry.amount_usd,
              status: 'paid',
              updated_at: new Date().toISOString(),
            }).eq('id', entry.id);
            links.push({ payment_id: payData.id, installment_entry_id: entry.id, allocated_amount_usd: allocated });
          } else {
            const allocated = Math.round(budget * 100) / 100;
            const newPaid = Math.round(((entry.paid_amount_usd || 0) + budget) * 100) / 100;
            await supabase.from('installment_entries').update({
        ...
        if (firstEntryId) {
          await supabase.from('payments').update({ installment_entry_id: firstEntryId }).eq('id', payData.id);
        }
        if (links.length > 0) {
    ...
    if (order) {
      ...
      if (formData.payment_type === 'deposit' && order.sale_type === 'installment') {
        const months = Number(order.installment_months || 6);
        ...
        if (existingSched?.id) {
          ...
          const { data: unpaidEntries } = await supabase
            ...
            .eq('schedule_id', existingSched.id)
            .eq('status', 'unpaid')
            .order('installment_number', { ascending: true });

          if (unpaidEntries && unpaidEntries.length > 0) {
            const unpaidCount = unpaidEntries.length;
            const baseAmount = Math.floor((remaining / unpaidCount) * 100) / 100;
            const totalBase = Math.round(baseAmount * unpaidCount * 100) / 100;
            const lastMonthExtra = Math.round((remaining - totalBase) * 100) / 100;
            const lastEntryId = unpaidEntries[unpaidEntries.length - 1].id;
            const regularIds = unpaidEntries.slice(0, -1).map((e: { id: string }) => e.id);
            if (regularIds.length > 0) {
              await supabase.from('installment_entries')
                .update({ amount_usd: baseAmount, updated_at: new Date().toISOString() })
                .in('id', regularIds);
            }
            await supabase.from('installment_entries')
              .update({ amount_usd: Math.round((baseAmount + lastMonthExtra) * 100) / 100, updated_at: new Date().toISOString() })
              .eq('id', lastEntryId);
          }
        }
      }
    ...
    if (activeSession && payData?.id) {
      ...
      const orderNum = (orderForLog as Record<string, unknown>)?.order_number as string || '';
  ...
  const handleReverse = async () => {
    ...
    await supabase.from('payments').update({ is_reversed: true, reversed_by: profile?.id }).eq('id', selectedPayment.id);

    if (selectedPayment.installment_entry_id) {
      const { data: entry } = await supabase.from('installment_entries').select('*').eq('id', selectedPayment.installment_entry_id).maybeSingle();
      if (entry) {
        const newPaid = Math.max(0, (entry.paid_amount_usd || 0) - selectedPayment.amount_usd);
        ...
        if (newStatus === 'unpaid' && entry.due_date < today) newStatus = 'overdue';
        await supabase.from('installment_entries').update({
          paid_amount_usd: newPaid,
          status: newStatus,
          updated_at: new Date().toISOString(),
    ...
    <div className="p-4 lg:p-6 space-y-5">
      {!activeSession && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
          ...
          <div>
            ...
            <p className="text-amber-700 text-xs">{t('openCashRegisterToRecord')}</p>
          </div>
        </div>
      )}
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={filterType} onChange={e => setFilterType(e.target.value)} className="px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none">
          ...
          {['deposit', 'installment', 'final', 'partial', 'reversal'].map(pt => (
            <option key={pt} value={pt}>{t(pt as Parameters<typeof t>[0])}</option>
          ))}
        </select>
        <input
          type="date"
        ...
        />
        {canCreate && (
          <Button onClick={() => setShowAddModal(true)} icon={<Plus size={16} />} disabled={!activeSession}>{t('addPayment')}</Button>
        )}
      </div>

      <p className="text-sm text-gray-500">{total} {t('payments').toLowerCase()}</p>
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-400">
                <div className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />{t('loading')}</div>
              </td></tr>
            ) : payments.length === 0 ? (
              <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-400">{t('noData')}</td></tr>
            ) : payments.map(pay => (
              <tr key={pay.id} className={`hover:bg-emerald-50/20 transition-colors ${pay.is_reversed ? 'opacity-50' : ''}`}>
                <td className="px-4 py-3 font-mono text-xs font-bold text-emerald-700">{pay.payment_number}</td>
                <td className="px-4 py-3 font-mono text-xs text-emerald-700">{(pay.order as Record<string, unknown>)?.order_number as string}</td>
                <td className="px-4 py-3 font-medium text-gray-900">
                  {language === 'ku'
                    ? ((pay.order as Record<string, unknown>)?.customer as Record<string, string>)?.full_name_ku
                    : ((pay.order as Record<string, unknown>)?.customer as Record<string, string>)?.full_name_en}
                </td>
                ...
                <td className="px-4 py-3 font-semibold text-gray-900">{fmt(pay.amount_usd)}</td>
                <td className="px-4 py-3 text-xs text-gray-700">
                  <div>{pay.currency === 'IQD' ? fmtIQD(pay.amount_in_currency) : fmt(pay.amount_in_currency)}</div>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    {canReverse && !pay.is_reversed && pay.payment_type !== 'reversal' && (
                      <button
                        onClick={() => { setSelectedPayment(pay); setReverseReason(''); setShowReverseModal(true); }}
                    ...
                    {pay.is_reversed && <Badge variant="error">{t('reversedBadge')}</Badge>}
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title={t('addPayment')} size="lg" footer={
        <div className="flex justify-end gap-3">
          <Button variant="secondary" onClick={() => setShowAddModal(false)}>{t('cancel')}</Button>
          <Button onClick={handleAddPayment} loading={saving} disabled={!formData.order_id || !formData.amount_in_currency}>{t('save')}</Button>
        </div>
      }>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('orders')} <span className="text-red-500">*</span></label>
            <select
            ...
            >
              ...
              {orders.map(o => (
                <option key={o.id} value={o.id}>
                  {(o as Record<string, unknown>).order_number as string} — {language === 'ku'
                    ? ((o as Record<string, unknown>).customer as Record<string, string>)?.full_name_ku
                    : ((o as Record<string, unknown>).customer as Record<string, string>)?.full_name_en}
                </option>
              ))}
            </select>
          </div>
          ...
          {selectedOrder && (
            <div className="p-3 bg-gray-50 rounded-xl text-xs space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{fmt(selectedOrder.final_total_usd)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">{t('depositRequired')} (50%):</span>
                <span className="font-semibold text-blue-700">{fmt(selectedOrder.deposit_required_usd || selectedOrder.final_total_usd * 0.5)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">{t('depositPaid')}:</span>
                <span className="font-semibold text-emerald-700">{fmt(selectedOrder.deposit_paid_usd || 0)}</span>
              </div>
              <div className="flex justify-between border-t border-gray-200 pt-1 mt-1">
                <span className="text-gray-500">{t('balanceDueLabel')}:</span>
                <span className={`font-bold ${Number(selectedOrder.balance_due_usd) > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                  {fmt(selectedOrder.balance_due_usd)}
                </span>
              </div>
              {(selectedOrder as Record<string, unknown>).sale_type === 'installment' && formData.payment_type === 'deposit' && (
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select
              ...
              onChange={e => setFormData(p => ({ ...p, payment_type: e.target.value as Payment['payment_type'] }))}
              options={['deposit', 'installment', 'final', 'partial'].map(pt => ({ value: pt, label: t(pt as Parameters<typeof t>[0]) }))}
            />
            <Select
              label={t('currency')}
              value={formData.currency}
              onChange={e => setFormData(p => ({ ...p, currency: e.target.value as Currency }))}
              options={[{ value: 'USD', label: 'USD' }, { value: 'IQD', label: 'IQD' }]}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input
              ...
              onChange={e => setFormData(p => ({ ...p, amount_in_currency: e.target.value }))}
              required
            />
            <Input
              label={t('date')}
          ...
          {formData.amount_in_currency && (
            <div className="p-3 bg-emerald-50 rounded-xl text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-emerald-700">{getExchangeRate().toLocaleString()} IQD / $1</span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-gray-600">{t('amountInUSD')}:</span>
                <span className="font-bold text-emerald-800 text-base">{fmt(getAmountUSD())}</span>
              </div>
              {formData.payment_type === 'installment' && (
                <p className="text-xs text-amber-600 mt-1">{t('installmentRateNote')}</p>
              )}
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('notesEn')}</label>
            <input
              value={formData.notes_en}
              onChange={e => setFormData(p => ({ ...p, notes_en: e.target.value }))}
              className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none"
            />
          </div>
          <div dir="rtl">
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('notesKu')}</label>
            <input
              value={formData.notes_ku}
              onChange={e => setFormData(p => ({ ...p, notes_ku: e.target.value }))}
      ...
      <Modal isOpen={showReverseModal} onClose={() => setShowReverseModal(false)} title={t('reverse')} size="sm" footer={
        <div className="flex justify-end gap-3">
          <Button variant="secondary" onClick={() => setShowReverseModal(false)}>{t('cancel')}</Button>
          <Button variant="danger" onClick={handleReverse} loading={saving} disabled={!reverseReason.trim()}>{t('reverse')}</Button>
        </div>
      }>
        {selectedPayment && (
          <div className="space-y-4">
            <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-sm space-y-1 text-red-700">
              <p>{t('reversingPayment')} <strong>{selectedPayment.payment_number}</strong></p>
              <p>{t('amount')}: <strong>{fmt(selectedPayment.amount_usd)}</strong></p>
      ...
      {receiptPayment && (() => {
        const ord = receiptPayment.order as (Order & { customer?: Customer }) | undefined;
