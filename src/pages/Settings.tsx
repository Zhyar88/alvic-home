  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      (data || []).forEach((s: any) => {
        ...
        if (s.key === 'max_discount_percent') setMaxDiscount(s.value);
      });
      setLoading(false);
    };
    fetchSettings();
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {language === "ku" ? "ڕێژەی پێشەکی نەقد" : "Cash Deposit Rate"}
              </label>
              <div className="flex items-center gap-3">
                <input
                  type="number"
...
<h2 className="text-base font-semibold text-gray-800 border-b border-gray-100 pb-3 mt-6">
            {language === 'ku' ? 'سنووری داشکاندن' : 'Discount Limit'}
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {language === 'ku' ? 'زۆرترین داشکاندن (%)' : 'Maximum Discount (%)'}
              </label>
              <input
                type="number"
                min={0}
              ...
              />
              <p className="text-xs text-gray-400 mt-1">
                {language === 'ku' ? 'زۆرترین ڕێژەی داشکاندنی مۆڵەتدراو لە داواکاریەکان' : 'Maximum allowed discount percentage on orders'}
              </p>
            </div>
          </div>
