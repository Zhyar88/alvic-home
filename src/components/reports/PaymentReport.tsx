...
export function PaymentReport() {
  ...
  const computeSummary = (rows: PaymentData[]) => {
    const countablePayments = rows.filter((p) => p.payment_status !== 'reversed');

    return countablePayments.reduce(
      (acc, p) => ({
        total_payments: acc.total_payments + 1,
        total_usd: acc.total_usd + Number(p.amount_usd || 0),
        total_iqd:
          acc.total_iqd + (p.currency === 'IQD' ? Number(p.amount_in_currency || 0) : 0),
        deposit_count: acc.deposit_count + (p.payment_type === 'deposit' ? 1 : 0),
        installment_count:
          acc.installment_count + (p.payment_type === 'installment' ? 1 : 0),
  ...
  try {
    ...
    // 1) orders
    const orderIds = [...new Set(rawPayments.map((p: any) => p.order_id).filter(Boolean))];
    ...
    // 3) detect reversed originals
    const reversedOriginalIds = new Set<string>(
      rawPayments
        .filter((p: any) => p.payment_type === 'reversal' && p.reversal_reference_id)
        .map((p: any) => p.reversal_reference_id as string)
    );

    // 4) keep only original payments in table
    const paymentData = rawPayments
      .filter((payment: any) => payment.payment_type !== 'reversal')
      .map((payment: any) => {
        const order = ordersMap[payment.order_id] || {};
        const customer = customersMap[order.customer_id] || {};
        const isReversedPayment =
          Boolean(payment.is_reversed) || reversedOriginalIds.has(payment.id);
        ...
        return {
          ...
          payment_status: isReversedPayment ? 'reversed' : 'active',
          is_reversed: payment.is_reversed,
          reversal_reference_id: payment.reversal_reference_id,
          order_id: order.id || '',
          ...
          amount_in_currency: Number(payment.amount_in_currency || 0),
    ...
    setSummary(computeSummary(paymentData));
  } catch (error) {
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            <option value="all">
              {language === 'ku' ? 'هەموو کڕیارەکان' : 'All Customers'}
            </option>
          ...
          >
            <option value="all">
              {language === 'ku' ? 'هەموو داواکارییەکان' : 'All Orders'}
            </option>
            {orders.map((order) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 border border-blue-200">
              ...
              <p className="text-xs text-blue-600 mt-1">
                {summary.total_payments} {language === 'ku' ? 'وەرگرتن' : 'payments'}
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4 border border-amber-200">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2 text-xs">
                <Badge variant="neutral">
                  {summary.installment_count} {language === 'ku' ? 'بەش' : 'Inst.'}
                </Badge>
                <Badge variant="warning">
                  {summary.final_count} {language === 'ku' ? 'کۆتایی' : 'Final'}
                </Badge>
                <Badge variant="info">
                  {summary.partial_count} {language === 'ku' ? 'بەشێک' : 'Part.'}
                </Badge>
              </div>
            </div>
          </div>
                  ...
                  {sortedPayments.map((payment) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900">
                        {language === 'ku'
                          ? payment.customer_name_ku
                          : payment.customer_name_en}
                      </td>
      ...
      {!loading && payments.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">
            {language === 'ku'
              ? 'تکایە فلتەرەکان هەڵبژێرە و دووگمەی پیشاندان دابگرە'
